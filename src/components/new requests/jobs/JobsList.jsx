    ...
    {Array.isArray(filteredJobs) && filteredJobs.length > 0 ? (
      filteredJobs.map((job) => (
        ...
        >
          ...
          {/* Action Button: link to non-dynamic job page using query param */}
          <Link href={`/dashboard/jobs?jobId=${encodeURIComponent(job._id)}`} className="flex items-center gap-2 justify-center md:justify-start px-4 py-3 md:py-2 text-sm font-medium text-[#541229] bg-[#54122915] hover:bg-[#54122925] rounded-lg transition-colors duration-200 group-hover:shadow-sm">
